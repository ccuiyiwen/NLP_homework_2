    ...
    for novel_file in os.listdir(corpus_dir):
        if novel_file.endswith('.txt'):
            ...
            for paragraph in novel_paragraphs:
                # 如果段落长度不超过num_paragraphs，则添加到数据集中
                words = word_tokenize(paragraph)  # 将段落分割成单词
                if len(words) <= max_tokens:
                    paragraphs.append(words)  # 以单词列表形式添加到数据集中
...
for k in max_tokens:
    ...
    for i, paragraph in enumerate(paragraphs):
        bow_vector = lda_model.id2word.doc2bow(paragraph)
    ...
    for train_index, test_index in kf.split(topics_distribution):
        ...
        # 构建LSTM模型
        model = Sequential([
            Embedding(input_dim=num_topics, output_dim=100, input_length=k),
